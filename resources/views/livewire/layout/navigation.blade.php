...
<nav x-data="{ open: false }">
    ...
    <div class="hidden sm:flex sm:items-center sm:ms-6 gap-4">
        ...
        @else
            ...
            <x-dropdown align="right" width="480">
                ...
                <x-slot name="content">
                    <div wire:poll.1s  class="p-4 dark:bg-gray-800 mt-[-15px] mb-[-15px]" style="min-width: 480px">
        ...
        {{-- <form method="POST" action="{{ route('currency.update')}}"> --}}
            <div class="inline-flex items-center gap-2 relative">
                <select
                    ...
                    class="bg-base-200 border border-gray-300 rounded-l-md px-4 py-2 text-sm text-gray-700 focus:outline-none focus:ring focus:ring-primary-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-primary-500">
                    @foreach (config('currencies.supported') as $code => $details)
                        <option value="{{ $code }}"
                            {{ auth()->user() && auth()->user()->currency
                                ? (auth()->user()->currency == $code ? 'selected' : '')
                                : (session('currency', config('currencies.default')) == $code ? 'selected' : '') }}>
                            {{ $details['name'] }}
                        </option>
                    @endforeach
                </select>
    ...
    <x-slot name="trigger">
        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150 dark:bg-gray-700 dark:hover:text-white dark:text-white">
            @if (Auth::check())
                {{-- Use the proxy route to display the avatar --}}
                @php
                    $avatarUrl = Auth::user()->avatar ? route('user.avatar', ['user' => Auth::user()->id]) : asset('user.png');
                @endphp
    ...
    <x-slot name="content">
        ...
        <button wire:click="logout" class="w-full text-start" type="submit">
            <x-dropdown-link>
                {{ __('Log Out') }}
            </x-dropdown-link>
        </button>
