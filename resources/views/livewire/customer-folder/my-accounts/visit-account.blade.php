                ...
                <x-mary-form wire:submit="withdraw" x-data="{ withdrawalMethod: 'default' }">
                    ...
                    <x-slot:actions>
                        {{-- @if($this->withdrawFromAccount) --}}
                            <x-wireui-button icon="arrow-up" label="Withdraw" wire:click="withdraw({{$account->id}})" class="bg-red-500 text-white text-sm px-3 py-1 focus:outline-none focus:ring-2 focus:ring-red-400" spinner="withdraw({{$account->id}})" />
                        {{-- @endif --}}
                    </x-slot:actions>
                </x-mary-form>
